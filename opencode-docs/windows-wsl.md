# Windows (WSL) - OpenCode Documentation
...
2. **Install OpenCode in WSL:** Once WSL is set up, open your WSL terminal and install OpenCode.
   ```
   curl -fsSL https://opencode.ai/install | bash
   ```
3. **Use OpenCode from WSL:** Navigate to your project directory (access Windows files via `/mnt/c/`, `/mnt/d/`, etc.) and run OpenCode.
   ```
...
- For the smoothest experience, consider cloning/copying your repo into the WSL filesystem (e.g., under `~/code/`) and running OpenCode there.
