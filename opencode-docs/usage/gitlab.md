# GitLab - OpenCode Documentation

# GitLab

Use OpenCode in GitLab issues and merge requests.

OpenCode integrates with your GitLab workflow through your GitLab CI/CD pipeline or with GitLab Duo. In both cases, OpenCode will run on your GitLab runners.

## GitLab CI

OpenCode works in a regular GitLab pipeline. You can build it into a pipeline as a CI component using the community-created CI/CD component — nagyv/gitlab-opencode.

### Features

- Use custom configuration per job.
...
1. Store your OpenCode authentication JSON as a File type CI environment variable under Settings > CI/CD > Variables. Mark them as "Masked and hidden".
...
commands:
  ...
  - |
    ...
    If you are asked to summarize an MR or issue, please post back a note to the MR/Issue.
...
**Explain an issue:** Add `@opencode explain this issue` in a GitLab issue. OpenCode will read the issue and reply with a clear explanation.
...
**Review merge requests:** Leave `@opencode review this merge request` on a GitLab merge request. OpenCode will review the merge request and provide feedback.
