# Ecosystem - OpenCode Documentation

# Ecosystem

Projects and integrations built with OpenCode.

A collection of community projects built on OpenCode. Want to add your project to this list? Submit a PR. You can also check out awesome-opencode and opencode.cafe.
...
| opencode-vibeguard | Redact secrets/PII into VibeGuard-style placeholders; restore locally |
| opencode-websearch-cited | Add native websearch support for supported providers with Google grounded style |
| opencode-pty | Enables AI agents to run background processes in a PTY |
...
| opencode-skillful | Allow OpenCode agents to lazy load prompts on demand with skill discovery |
...
| @openspoon/subtask2 | Extend opencode /commands into a powerful orchestration system |
| opencode-scheduler | Schedule recurring jobs using launchd (Mac) or systemd (Linux) |
...
| opencode-background-agents | Claude Code-style background agents with async delegation |
| opencode-notify | Native OS notifications for OpenCode |
| opencode-workspace | Bundled multi-agent orchestration harness |
...
| opencode.nvim | Neovim frontend for opencode |
